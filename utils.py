...
def image2neighbourhoodMatrix(input: np.ndarray, kernel_shape:tuple, stride:int, padding:int)-> tuple:
    ...
    ii, jj, kk = get_neighborhood_indeces(input.shape, kernel_shape, stride, padding)
...
def neighborhoodMatrix2image(transformed_input:np.ndarray, input_shape:tuple, kernel_shape:tuple,
    ...
    # we then used a particular set of indeces to transform 4D input into 2D matrix whith each neighborhood
    ...
    input = transformed_input.reshape(C * Hk * Wk, -1, Nk).transpose(2, 0, 1)

    # populate the input image with the corresponding entries
    np.add.at(input_padded, (slice(None), kk, ii, jj), input)
...
def correlation2d(input:np.ndarray, kernel:np.ndarray, bias:np.ndarray, stride:int = 1, padding:int = 0) -> np.ndarray:
    ...
    transformed_input = image2neighbourhoodMatrix(input, kernel, stride, padding) # reshape input from B x C x H x W to (Ck*Hk*Wk)x(B*H*W)
    out = transformed_kernel @ transformed_input + bias

    # reshape to proper form
    out = out.reshape(Nk, Hout, Wout, B)

    # return cache to compute derivatives in backward pass (no need to store transformed_kernel as it is fastly computed)
    cache = input, transformed_input, kernel, bias, stride, padding
